...
-- 15. List total number of intermediate bus stops of each route. Rename the column to 'Number of Stops' (1234 rows)
-- SELECT route, bound, COUNT(stop) AS "Number of Stops" FROM routestop GROUP BY route, bound;

-- 16. List all stop ID, english name with 'Cross Harbour Tunnel' (12 rows)
-- SELECT stop, name_en FROM stops WHERE name_en="HUNG HOM CROSS HARBOUR TUNNEL TOLL PLAZA";

-- 17. From Q16, match all routes to corresponding stop of 'Cross Harbour Tunnel' (126 rows)
-- SELECT route, routestop.bound, routestop.stop, name_en FROM stops INNER JOIN routestop ON stops.stop=routestop.stop WHERE name_en="HUNG HOM CROSS HARBOUR TUNNEL TOLL PLAZA";

-- 18. From Q17, list results in ascending order of stop ID (126 rows)
-- SELECT route, routestop.bound, routestop.stop, name_en FROM stops INNER JOIN routestop ON stops.stop=routestop.stop WHERE name_en="HUNG HOM CROSS HARBOUR TUNNEL TOLL PLAZA" ORDER BY stop ASC;

-- 19. From Q18, clean up the data by avoiding duplicate records display (84 rows)
-- SELECT DISTINCT route, routestop.bound, routestop.stop, name_en FROM stops INNER JOIN routestop ON stops.stop=routestop.stop WHERE name_en="HUNG HOM CROSS HARBOUR TUNNEL TOLL PLAZA" ORDER BY stop ASC;
