...
def upload():
    ...
    if picture.filename == '':
        ...
        picture.save(os.path.join(app.config["UPLOAD_FOLDER"], ))
    with open("blog.json", "r") as f:
        blog = json.load(f)
    blog["data"][username] = {"title": title, "description": shortDescription, "catagory": catagory, "photo": picture, "content": actualContent}
