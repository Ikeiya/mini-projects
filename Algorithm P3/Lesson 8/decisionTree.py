...
for i in range(len(entropyVal)):
    temp = []
    tempTotal = sum(entropyVal[i])
    if i == 0:
        total = 0
        for j in range(len(entropyVal)):
            total += sum(entropyVal[j])
    for j in range(len(entropyVal[i])):
        temp.append(entropyVal[i][j]/tempTotal)
    entropyVal[i] = temp
    temp = 0
    for j in range(len(entropyVal[i])):
        if entropyVal[i][j] != 0:
            temp -= entropyVal[i][j]*(math.log(entropyVal[i][j], 2))
    print(entropyVal[i], tempTotal,total)
    entropyVal[i] = temp*tempTotal/total
