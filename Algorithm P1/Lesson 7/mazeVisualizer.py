...
def pathfinder(maze1, path, coordinate):
	result = nodeCheck(maze1, path, coordinate)
	for i in range(len(result)):
		if result[0] == "found":
			print('jjj')
			return path
		path.append(result[i])
		maze.setCell(result[i][1], result[i][0], "V")
		maze.show()
		pathfinder(maze1, path, result[i])

# Example
if __name__ == "__main__":
	colors = {	# The corresponding color for each symbol (example)
		" ": "#008000", # Road
		"S": "#ff2d00", # Starting Point
		"E": "#6eff40", # Ending Point
		"0": "#404040", # Wall
		"V": "#808000", # Visited Cells
		"C": "#008000"  # Currently in the stack/queue
	}
	maze = Maze("mazeD0.txt", colors, gridSize=5, FPS=10)
